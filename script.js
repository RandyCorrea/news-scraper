...
async function loadNews() {
    try {
        ...
        // Filter out locally discarded items (to hide them instantly while GitHub syncs)
        const localDiscarded = JSON.parse(localStorage.getItem('discarded_ids') || '[]');
        newsData = fetchedData.filter(item => !localDiscarded.includes(String(item.id)));
...
window.discardArticle = async function (id) {
    ...
    // 1. Optimistic Local Hide
    const article = newsData.find(a => String(a.id) === String(id));
    if (article) {
        // Save to local storage blacklist
        const localDiscarded = JSON.parse(localStorage.getItem('discarded_ids') || '[]');
        if (!localDiscarded.includes(String(id))) {
            localDiscarded.push(String(id));
            localStorage.setItem('discarded_ids', JSON.stringify(localDiscarded));
        }

        // Remove from current view
        newsData = newsData.filter(a => String(a.id) !== String(id));
        ...
        // 2. Sync to GitHub (Background)
        try {
            await updateGitHubFile(NEWS_SOURCE, newsData, `Discard article ${id}`);
        } catch (e) {
            console.error(e);
            showToast('Syncing to cloud (it might take a minute)...', 'info');
        }
    }
};
